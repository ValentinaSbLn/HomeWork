...
public class Factory implements Factorable {
    ...
    private void postInit() throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException, InvocationTargetException {
        ...
        for (Class<?> c : classes) {
            List<Object> al = beans.get(c);
            for (int i = 0; i < al.size(); i++) {
                Method[] methods = al.get(i).getClass().getDeclaredMethods();
...
//        }
    ...
    private Class<?> findBeanClsFor(Class<?> d, Map<Class<?>, List<Class<?>>> beansTypes) {
        ...
        for (Map.Entry<Class<?>, List<Class<?>>> i : beansTypes.entrySet()) {
            ...
            i.getValue().stream().filter(new Predicate<Class<?>>() {
                @Override
    ...
    public void close() {
        ...
        try {
            classes = getAllClassesByPredicate(baseFile, c ->
                    c.isAnnotationPresent(Component.class));
            for (Class<?> c : classes) {
                Method[] methods = c.getDeclaredMethods();
            ...
            throw new RuntimeException("The object can not be closed");
        }
